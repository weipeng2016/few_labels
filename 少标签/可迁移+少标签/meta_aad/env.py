...
class TrainEnv(EnvBase):
    ...
    def step(self, action):
        ...
        if action == 0 and self._Y[self.pointer] == 0:
            r = 0
            self.normalies.append(self.pointer)
        elif action == 0 and self._Y[self.pointer] == 1:
            r = -0.1
            self.anomalies.append(self.pointer)
        elif action == 1 and self._Y[self.pointer] == 1:
            r = 1
            self.anomalies.append(self.pointer)
            self.count += 1
        elif action == 1 and self._Y[self.pointer] == 0:
            r = -0.1
            self.normalies.append(self.pointer)
            self.count += 1
        else:
        ...
        if self._Y[self.pointer] == -1:
            if self.pointer < 500:
                ...
                r += self._scores[self.pointer].item()
                #print(self._scores[self.pointer].item(), type(self._scores[self.pointer].item()))
                #sys.exit()#self.clf.predict()
            elif self.pointer % 500 == 0:
                self.clf.fit(self._obs_for_ifoest())
                #print(self.clf.decision_function(np.array(self._obs()).reshape(1, -1)).item())
                #print(type(self.clf.decision_function(np.array(self._obs()).reshape(1, -1)).item()))
                #sys.exit()
                r += self.clf.decision_function(np.array(self._obs()).reshape(1, -1)).item()
